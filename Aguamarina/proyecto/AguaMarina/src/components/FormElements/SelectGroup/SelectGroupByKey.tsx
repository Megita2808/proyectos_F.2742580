...
interface SelectGroupProps {
  ...
  placeholder?: string;
  required?: boolean;
  onChange?: (event: React.ChangeEvent<HTMLSelectElement>, key: string | number) => void;
}

const SelectGroupByKey: React.FC<SelectGroupProps> = ({
  id,
  name,
  value,
  customClasses,
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    changeTextColor();
    if (onChange) {
      // Pasa el 'key' de la opción seleccionada
      const selectedOptionKey = opciones.find((opc) => String(opc.id) === selectedValue)?.id;
      if (selectedOptionKey !== undefined) {
        onChange(e, selectedOptionKey); // Llama a la función onChange pasando el key
      }
    }
  };
  ...
  return (
    <div className={`mb-4.5 cursor-pointer ${customClasses}`}>
      ...
      <div className="relative z-20 bg-transparent rounded-[7px] dark:bg-dark-2 cursor-pointer">
        ...
        >
          <option value="0" className="text-dark-6 font-bold text-lg">
            {placeholder}
          </option>
          {opciones.map((opc) => (
...
export default SelectGroupByKey;
